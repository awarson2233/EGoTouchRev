  ...
  try {
    Interceptor.attach(target, {
      onEnter(args) {
        ...
        if (exportName === 'TSA_ASAProcess' && arg0 && !arg0.isNull()) {
            try {
                evt.struct_dump = {
                    TimestampSec: Memory.readU32(arg0.add(0x00)),
                    TimestampUsec: Memory.readU32(arg0.add(0x04)),
                    SequenceId: Memory.readU16(arg0.add(0x08)),
                    TouchMatrixPtr: ptrToString(Memory.readPointer(arg0.add(0x10))),
                    HwStatus0: Memory.readU16(arg0.add(0x30)),
                    HwStatus1: Memory.readU16(arg0.add(0x32)),
                    StatusFlags: Memory.readU32(arg0.add(0x34)),
                    StylusMatrixPairPtr: ptrToString(Memory.readPointer(arg0.add(0x40)))
                };
                
                // Read extra bytes for padding to see if they are 0s
                evt.struct_dump_raw = [];
                for(let i=0; i<0x58; i++) {
                   evt.struct_dump_raw.push(Memory.readU8(arg0.add(i)));
                }
            } catch(e) {
                evt.struct_dump_err = String(e);
            }
        }

        // ────────────────────────────────────────────────────────────
        // THP_AFE_FRAME 结构体验证 (TSA_ProcessingAFEFrame)
        // 文档参考: docs/Himax_Thp_Frame_Flow.md  §2
        //
        // 布局 (总计 0x58 = 88 字节):
        //   0x00  uint32_t   TimestampSec
        //   0x04  uint32_t   TimestampUsec
        //   0x08  uint16_t   SequenceId
        //   0x0A  uint8_t[6] _Padding0         (应全零)
        //   0x10  void*      TouchMatrixPtr    [重点]
        //   0x18  void*      _ReservedPtr1
        //   0x20  void*      _ReservedPtr2
        //   0x28  void*      _ReservedPtr3
        //   0x30  uint16_t   HwStatus0
        //   0x32  uint16_t   HwStatus1
        //   0x34  uint32_t   StatusFlags
        //   0x38  uint64_t   _Padding1         (应全零)
        //   0x40  void**     StylusMatrixPairPtr [重点]
        //   0x48  void*      _ReservedPtr4
        //   0x50  void*      _ReservedPtr5
        // ────────────────────────────────────────────────────────────
        if (exportName === 'TSA_ProcessingAFEFrame' && arg0 && !arg0.isNull()) {
            var _diagStep = 'init';
            var _diagInfo = {};
            try {
                var ptrSz = Process.pointerSize;
                _diagStep = 'ptrConvert';
                var p = ptr(arg0.toString());

                // 诊断: 检查 Memory API 可用性
                _diagInfo.typeofMemory = typeof Memory;
                _diagInfo.typeofReadU32 = typeof Memory.readU32;
                _diagInfo.typeofReadByteArray = typeof Memory.readByteArray;
                _diagInfo.ptrHasReadU32 = typeof p.readU32;
                _diagInfo.pAddr = p.toString();

                _diagStep = 'readFields';
                var tsSec       = p.readU32();
                var tsUsec      = p.add(0x04).readU32();
                var seqId       = p.add(0x08).readU16();

                _diagStep = 'readPad0';
                var pad0Bytes = [];
                var pi = 0;
                for (pi = 0; pi < 6; pi++) {
                    pad0Bytes.push(p.add(0x0A + pi).readU8());
                }

                _diagStep = 'readPtrs1';
                var touchMatrixPtr      = p.add(0x10).readPointer();
                var reservedPtr1        = p.add(0x18).readPointer();
                var reservedPtr2        = p.add(0x20).readPointer();
                var reservedPtr3        = p.add(0x28).readPointer();

                _diagStep = 'readHwStatus';
                var hwStatus0           = p.add(0x30).readU16();
                var hwStatus1           = p.add(0x32).readU16();
                var statusFlags         = p.add(0x34).readU32();

                _diagStep = 'readPad1';
                var pad1Bytes = [];
                for (pi = 0; pi < 8; pi++) {
                    pad1Bytes.push(p.add(0x38 + pi).readU8());
                }

                _diagStep = 'readPtrs2';
                var stylusMatrixPairPtr = p.add(0x40).readPointer();
                var reservedPtr4        = p.add(0x48).readPointer();
                var reservedPtr5        = p.add(0x50).readPointer();
                ...
                var frameDump = {
                    ...
                    SequenceId:            seqId,
                    _Padding0:             pad0Bytes,
                    TouchMatrixPtr:        touchMatrixPtr.toString(),
                    _ReservedPtr1:         reservedPtr1.toString(),
                    _ReservedPtr2:         reservedPtr2.toString(),
                    _ReservedPtr3:         reservedPtr3.toString(),
                    HwStatus0:             hwStatus0,
                    HwStatus1:             hwStatus1,
                    StatusFlags:           '0x' + statusFlags.toString(16),
                    _Padding1:             pad1Bytes,
                    StylusMatrixPairPtr:   stylusMatrixPairPtr.toString(),
                    _ReservedPtr4:         reservedPtr4.toString(),
                    _ReservedPtr5:         reservedPtr5.toString()
                };
                ...
                // 1. _Padding0 check
                var pad0NonZero = false;
                for (pi = 0; pi < pad0Bytes.length; pi++) {
                    if (pad0Bytes[pi] !== 0) { pad0NonZero = true; break; }
                }
                if (pad0NonZero) {
                    issues.push('_Padding0 non-zero: [' + pad0Bytes.join(',') + ']');
                }

                // 2. _Padding1 check
                var pad1NonZero = false;
                for (pi = 0; pi < pad1Bytes.length; pi++) {
                    if (pad1Bytes[pi] !== 0) { pad1NonZero = true; break; }
                }
                if (pad1NonZero) {
                    issues.push('_Padding1 non-zero: [' + pad1Bytes.join(',') + ']');
                }

                // 3. TimestampUsec range
                if (tsUsec > 999999) {
                    issues.push('TimestampUsec=' + tsUsec + ' >999999');
                }

                // 4. TimestampSec sanity
                if (tsSec !== 0 && (tsSec < 1577836800 || tsSec > 2208988800)) {
                    issues.push('TimestampSec=' + tsSec + ' out of range');
                }

                // 5. TouchMatrixPtr null check
                if (touchMatrixPtr.isNull()) {
                    issues.push('TouchMatrixPtr is NULL');
                }

                // 6. StylusMatrixPairPtr null check
                if (stylusMatrixPairPtr.isNull()) {
                    issues.push('StylusMatrixPairPtr is NULL');
                }

                // 7. StatusFlags known-value check
                if (statusFlags !== 0x00 && statusFlags !== 0x40 && statusFlags !== 0x02) {
                    issues.push('StatusFlags=0x' + statusFlags.toString(16) + ' unknown');
                }
                _diagStep = 'deref';

                // 8. Dereference StylusMatrixPairPtr for F0/F1
                var stylusDeref = null;
                if (!stylusMatrixPairPtr.isNull()) {
                    try {
                        var f0Ptr = stylusMatrixPairPtr.readPointer();
                        var f1Ptr = stylusMatrixPairPtr.add(ptrSz).readPointer();
                        stylusDeref = {
                            F0Ptr: f0Ptr.toString(),
                            F1Ptr: f1Ptr.toString(),
                            F0Null: f0Ptr.isNull(),
                            F1Null: f1Ptr.isNull()
                        };
                        if (f0Ptr.isNull()) {
                            issues.push('F0 is NULL');
                        } else {
                            var f0Data = [];
                            for (var mi = 0; mi < 81; mi++) {
                                f0Data.push(f0Ptr.add(mi * 2).readS16());
                            }
                            stylusDeref.F0Data = f0Data;
                        }
                        if (f1Ptr.isNull()) {
                            issues.push('F1 is NULL');
                        } else {
                            var f1Data = [];
                            for (var mi = 0; mi < 81; mi++) {
                                f1Data.push(f1Ptr.add(mi * 2).readS16());
                            }
                            stylusDeref.F1Data = f1Data;
                        }
                    } catch (derefErr) {
                        issues.push('deref fail: ' + String(derefErr));
                    }
                }
                _diagStep = 'writeResult';

                writeEvent({
                    kind: 'struct-validate',
                    exportName: exportName,
                    structName: 'THP_AFE_FRAME',
                    structSize: '0x58',
                    baseAddr: p.toString(),
                    fields: frameDump,
                    stylusDeref: stylusDeref,
                    rawBytes: rawBytes,
                    validation: {
                ...
                evt.afe_frame = frameDump;
                evt.afe_frame_valid = issues.length === 0;
                if (issues.length > 0) {
                    evt.afe_frame_issues = issues;
                }
                _diagStep = 'done';
            } catch (e) {
                evt.afe_frame_err = String(e);
                evt.afe_frame_failStep = _diagStep;
                evt.afe_frame_diag = _diagInfo;
            }
        }
