...
#include "StylusSolver/shared/CoorIIRProcess.hpp"
#include "StylusSolver/shared/CoorSpeedProcess.hpp"
#include "StylusSolver/shared/LinearFilterProcess.hpp"
...
auto ProcessHpp3(ProcessT& process, Solvers::HeatmapFrame& frame) {
    Solvers::Stylus::Hpp3::Settings settings{};
    Solvers::Stylus::Hpp3::State state{};
    auto& runtime = frame.stylus.runtime.SelectHpp3();
    Solvers::Stylus::Hpp3::Context ctx{frame, runtime, settings, state};
...
Asa::CoorResult Coor(int32_t dim1, int32_t dim2) {
    Asa::CoorResult coor{};
    coor.valid = true;
    coor.dim1 = dim1;
    coor.dim2 = dim2;
...
Asa::CoorResult Feed(Solvers::Stylus::LinearFilterProcess& filter,
                        int32_t dim1,
                        int32_t dim2,
                        bool pressure = true) {
...
void FillSlaveSuffixBlock(uint16_t* block, uint16_t anchorRow, uint16_t anchorCol, int16_t baseValue) {
    block[0] = anchorRow;
    block[1] = anchorCol;
    for (int r = 0; r < Solvers::Stylus::Hpp3::kGridDim; ++r) {
        for (int c = 0; c < Solvers::Stylus::Hpp3::kGridDim; ++c) {
            block[2 + r * Solvers::Stylus::Hpp3::kGridDim + c] = static_cast<uint16_t>(baseValue + r * Solvers::Stylus::Hpp3::kGridDim + c);
        }
    }
}

void TestExtractGridFromSlavePayloadBytesMatchesWords() {
    static constexpr std::size_t wordCount = static_cast<std::size_t>(Solvers::Stylus::Hpp3::kBlockWords * 2);
    ...
    for (int r = 0; r < Solvers::Stylus::Hpp3::kGridDim; ++r) {
        for (int c = 0; c < Solvers::Stylus::Hpp3::kGridDim; ++c) {
            words[static_cast<std::size_t>(2 + r * Solvers::Stylus::Hpp3::kGridDim + c)] =
                static_cast<uint16_t>(100 + r * Solvers::Stylus::Hpp3::kGridDim + c);
        }
    }

    const std::size_t tx2Base = static_cast<std::size_t>(Solvers::Stylus::Hpp3::kBlockWords);
    words[tx2Base] = 7;
    words[tx2Base + 1] = 8;
    for (int r = 0; r < Solvers::Stylus::Hpp3::kGridDim; ++r) {
        for (int c = 0; c < Solvers::Stylus::Hpp3::kGridDim; ++c) {
            words[tx2Base + static_cast<std::size_t>(2 + r * Solvers::Stylus::Hpp3::kGridDim + c)] =
                static_cast<uint16_t>(1000 + r * Solvers::Stylus::Hpp3::kGridDim + c);
        }
    }
    ...
    const Solvers::Stylus::Hpp3::GridData fromWords = Solvers::Stylus::Hpp3::ExtractGridFromSlaveWords(words.data(), static_cast<int>(words.size()));
    const Solvers::Stylus::Hpp3::GridData fromBytes = Solvers::Stylus::Hpp3::ExtractGridFromSlavePayloadBytes(bytes.data(), bytes.size());
...
void TestFrameParserUsesSlaveSuffixWhenRawBytesMissing() {
    ...
    FillSlaveSuffixBlock(frame.slaveSuffix.words + Solvers::Stylus::Hpp3::kBlockWords, 7, 8, 1000);

    Solvers::Stylus::StylusFrameParser parser;
    parser.Process(frame);

    const auto& stylus = frame.stylus;
    Require(stylus.runtime.Active().parse.valid,
            "parser should accept slaveSuffix when raw bytes are missing");
    Require(stylus.runtime.Active().flow.frameClass == Asa::FrameClass::Valid,
            "slaveSuffix fallback should classify valid TX1 data as a valid frame");
    Require(stylus.runtime.hpp3.rawGrid.grid.tx1.anchorRow == 3 &&
            stylus.runtime.hpp3.rawGrid.grid.tx1.anchorCol == 4 &&
            stylus.runtime.hpp3.rawGrid.grid.tx1.grid[4][5] == 141,
            "slaveSuffix fallback should populate TX1 raw grid");
    Require(stylus.runtime.hpp3.rawGrid.grid.tx2.anchorRow == 7 &&
            stylus.runtime.hpp3.rawGrid.grid.tx2.anchorCol == 8 &&
            stylus.runtime.hpp3.rawGrid.grid.tx2.grid[1][2] == 1011,
...
void TestFrameParserReportsNoSignalForInvalidSlaveSuffixAnchors() {
    Solvers::HeatmapFrame frame{};
    frame.slaveSuffixValid = true;
    frame.slaveSuffix.words[0] = Solvers::Stylus::Hpp3::kAnchorInvalid;
    frame.slaveSuffix.words[1] = Solvers::Stylus::Hpp3::kAnchorInvalid;
    frame.slaveSuffix.words[Solvers::Stylus::Hpp3::kBlockWords] = Solvers::Stylus::Hpp3::kAnchorInvalid;
    frame.slaveSuffix.words[Solvers::Stylus::Hpp3::kBlockWords + 1] = Solvers::Stylus::Hpp3::kAnchorInvalid;
    ...
    Require(frame.stylus.runtime.Active().flow.frameClass == Asa::FrameClass::NoSignal,
            "invalid slaveSuffix anchors should classify as NoSignal");
    Require(!frame.stylus.runtime.Active().parse.valid && !frame.stylus.runtime.Active().parse.hasCurrentStylusSignal,
            "invalid slaveSuffix anchors should not publish a valid parse");
}

void SetProjectionPeak(Solvers::Stylus::Hpp3::Projection& projection,
                       int peakDim1,
                       int peakDim2,
                       int leftDim1,
                       int centerDim1,
...
void SetTx1LinePeakFrame(Solvers::HeatmapFrame& frame,
    ...
    stylus.runtime.Active().parse.valid = true;
    stylus.runtime.hpp3.rawGrid.grid.tx1.valid = true;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorRow = 4;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorCol = 4;
    stylus.runtime.hpp3.rawGrid.grid.tx1.grid[4][leftCol] = static_cast<int16_t>(leftValue);
    if (rightCol >= 0) {
        stylus.runtime.hpp3.rawGrid.grid.tx1.grid[4][rightCol] = static_cast<int16_t>(rightValue);
    }
}
...
void TestBypassModeResetsPostFilter() {
    ...
    Solvers::HeatmapFrame frame{};
    auto& stylus = frame.stylus;
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(1234, 2345);
    stylus.runtime.Active().decision.inRangeCandidate = true;
    stylus.runtime.Active().decision.tipDownCandidate = true;
    stylus.runtime.Active().pressure.outputPressure = 300;

    filter.Process(frame);

    Require(stylus.runtime.Active().post.finalCoor.dim1 == 1234 &&
            stylus.runtime.Active().post.finalCoor.dim2 == 2345,
            "passthrough should copy raw coordinate to post output on first frame");
    Require(stylus.runtime.Active().post.linearFilterState == 1,
...
void TestGridFeatureExtractorAlignsTx2WithFactoryFlow() {
    ...
    stylus.runtime.Active().parse.valid = true;
    stylus.runtime.hpp3.rawGrid.grid.tx1.valid = true;
    stylus.runtime.hpp3.rawGrid.grid.tx2.valid = true;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorRow = 4;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorCol = 4;
    stylus.runtime.hpp3.rawGrid.grid.tx2.anchorRow = 4;
    stylus.runtime.hpp3.rawGrid.grid.tx2.anchorCol = 4;
    stylus.runtime.hpp3.rawGrid.grid.tx1.grid[4][4] = 200;
    stylus.runtime.hpp3.rawGrid.grid.tx2.grid[4][4] = 130;
    stylus.runtime.hpp3.rawGrid.grid.tx2.grid[5][4] = 150;

    Solvers::Stylus::Hpp3::GridFeatureExtractor extractor;
    ProcessHpp3(extractor, frame);

    const auto& tx2 = stylus.runtime.hpp3.tx2Grid.feature;
...
void TestTx2SeedThresholdIsGreaterThanNinetyNine() {
    ...
    low.runtime.Active().parse.valid = true;
    low.runtime.hpp3.rawGrid.grid.tx1.valid = true;
    low.runtime.hpp3.rawGrid.grid.tx2.valid = true;
    low.runtime.hpp3.rawGrid.grid.tx1.anchorRow = 0;
    low.runtime.hpp3.rawGrid.grid.tx1.anchorCol = 0;
    low.runtime.hpp3.rawGrid.grid.tx2.anchorRow = 4;
    low.runtime.hpp3.rawGrid.grid.tx2.anchorCol = 4;
    low.runtime.hpp3.rawGrid.grid.tx1.grid[0][0] = 200;
    low.runtime.hpp3.rawGrid.grid.tx2.grid[4][4] = 99;

    Solvers::Stylus::Hpp3::GridFeatureExtractor extractor;
    ProcessHpp3(extractor, lowFrame);
    Require(low.runtime.hpp3.tx2Grid.feature.peakTable.count == 0,
            "TX2 candidate value 99 should not seed a peak");

    Solvers::HeatmapFrame highFrame{};
    auto& high = highFrame.stylus;
    high.runtime.Active().parse.valid = true;
    high.runtime.hpp3.rawGrid.grid.tx1.valid = true;
    high.runtime.hpp3.rawGrid.grid.tx2.valid = true;
    high.runtime.hpp3.rawGrid.grid.tx1.anchorRow = 0;
    high.runtime.hpp3.rawGrid.grid.tx1.anchorCol = 0;
    high.runtime.hpp3.rawGrid.grid.tx2.anchorRow = 4;
    high.runtime.hpp3.rawGrid.grid.tx2.anchorCol = 4;
    high.runtime.hpp3.rawGrid.grid.tx1.grid[0][0] = 200;
    high.runtime.hpp3.rawGrid.grid.tx2.grid[4][4] = 100;

    ProcessHpp3(extractor, highFrame);
    Require(high.runtime.hpp3.tx2Grid.feature.peakTable.count == 1,
...
void TestTx1LinePeakFirstFrameUsesStrongestCurrentPeak() {
    ...
    Require(frame.stylus.runtime.hpp3.tx1Grid.feature.projection.peakIdxDim1 == 3,
            "first TX1 line peak frame should select the strongest current dim1 peak");
    Require(frame.stylus.runtime.hpp3.tx1Grid.feature.projection.peakIdxDim2 == 4,
...
void TestTx1LinePeakHistoryCarriesPreviousSelection() {
    ...
    ProcessHpp3(extractor, first);
    Require(first.stylus.runtime.hpp3.tx1Grid.feature.projection.peakIdxDim1 == 3,
    ...
    ProcessHpp3(extractor, second);
    Require(second.stylus.runtime.hpp3.tx1Grid.feature.projection.peakIdxDim1 == 3,
...
void TestTx1LinePeakHistoryDoesNotCarryDuringHover() {
    ...
    SetTx1LinePeakFrame(second, 3, 520, 6, 560);
    ProcessHpp3(extractor, second);
    Require(second.stylus.runtime.hpp3.tx1Grid.feature.projection.peakIdxDim1 == 6,
...
void TestPhysicalLowEdge3x3SumUsesInwardCells() {
    ...
    stylus.runtime.Active().parse.valid = true;
    stylus.runtime.hpp3.rawGrid.grid.tx1.valid = true;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorRow = 10;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorCol = 0;

    for (int r = 3; r <= 5; ++r) {
        stylus.runtime.hpp3.rawGrid.grid.tx1.grid[r][3] = 50;
        stylus.runtime.hpp3.rawGrid.grid.tx1.grid[r][4] = static_cast<int16_t>(r == 4 ? 300 : 250);
        stylus.runtime.hpp3.rawGrid.grid.tx1.grid[r][5] = 100;
        stylus.runtime.hpp3.rawGrid.grid.tx1.grid[r][6] = 70;
    }

    Solvers::Stylus::Hpp3::GridFeatureExtractor extractor;
    ProcessHpp3(extractor, frame);

    Require(stylus.runtime.hpp3.tx1Grid.feature.peak.valid,
            "physical low-edge fixture should produce a TX1 peak");
    Require(stylus.runtime.hpp3.tx1Grid.feature.peak.neighborSum3x3 == 1310,
...
void TestCoordinateSolverDoesNotMarkLocalEdgeAsPhysicalEdge() {
    Solvers::HeatmapFrame frame{};
    auto& stylus = frame.stylus;
    stylus.runtime.hpp3.tx1Grid.feature.peak.valid = true;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorRow = 10;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorCol = 10;

    auto& projection = stylus.runtime.hpp3.tx1Grid.feature.projection;
    projection.Clear();
    ...
    Solvers::Stylus::Hpp3::CoordinateSolver solver;
    ProcessHpp3(solver, frame);

    Require(stylus.runtime.Active().tx1.coordinate.localGridCoor.valid,
            "local-window edge should still solve using missing-neighbor zeroes");
    Require(!stylus.runtime.Active().signal.dim1EdgeActive,
...
void TestCoordinateSolverPublishesTx1AxisEdgeSignals() {
    ...
    auto& stylus = frame.stylus;
    stylus.runtime.hpp3.tx1Grid.feature.peak.valid = true;
    stylus.runtime.hpp3.tx1Grid.feature.peakSignal = 4000;
    stylus.runtime.hpp3.tx2Grid.feature.peakSignal = 0;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorRow = 0;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorCol = 0;
    ...
    stylus.runtime.hpp3.tx1Grid.feature.dim1SelectedPeakOnEdge = true;
    stylus.runtime.hpp3.tx1Grid.feature.dim2SelectedPeakOnEdge = true;
    stylus.runtime.hpp3.tx1Grid.feature.dim1SelectedPeakNetSignal = 2200;
    stylus.runtime.hpp3.tx1Grid.feature.dim2SelectedPeakNetSignal = 2300;

    Solvers::Stylus::Hpp3::CoordinateSolver solver;
    ProcessHpp3(solver, frame);

    Require(stylus.runtime.Active().signal.signalY == 0,
            "TX2 peak signal should remain available separately for tilt");
    Require(stylus.runtime.Active().signal.dim1EdgeSignal == 2200 &&
            stylus.runtime.Active().signal.dim2EdgeSignal == 2300,
            "edge pressure signals should come from TX1 dim1/dim2 line peaks");

    Solvers::Stylus::Hpp3::Hpp3PostPressureProcess postPressure;
    auto& pressure = stylus.runtime.Active().pressure;
    pressure.outputPressure = 200;
    pressure.rawPressure = 200;
    pressure.mappedPressure = 200;
    stylus.runtime.Active().decision.tipDownCandidate = true;
    stylus.runtime.Active().decision.authoritativeDown = true;
    ProcessHpp3(postPressure, frame);

    Require(stylus.runtime.Active().pressure.outputPressure == 200,
...
void TestNoisePostRejectsJumpBeforeTiltProcess() {
    Solvers::Stylus::Hpp3::Hpp3NoisePostProcess noise;
    Solvers::HeatmapFrame frame{};
    auto& runtime = frame.stylus.runtime.SelectHpp3();
    runtime.tx1.coordinate.reportGlobalCoor = Coor(10000, 10000);
    runtime.tx2Grid.feature.refinedLocalCoor = Coor(0, 0);
    runtime.rawGrid.grid.tx2.valid = true;
    runtime.rawGrid.grid.tx2.anchorRow = 4;
    runtime.rawGrid.grid.tx2.anchorCol = 4;
    runtime.signal.signalX = 1000;
    runtime.signal.signalY = 1000;

    ProcessHpp3(noise, frame);
...
void TestAftCoorProcessScalesFactoryLockThresholds() {
    ...
    Solvers::HeatmapFrame frame{};
    auto& stylus = frame.stylus;
    stylus.runtime.Active().pressure.outputPressure = 100;
    stylus.runtime.Active().post.finalCoor = Coor(100, 100);
    aft.Process(frame);

    stylus.runtime.Active().post.finalCoor = Coor(120, 100);
    aft.Process(frame);
    Require(stylus.runtime.Active().post.finalCoor.dim1 == 100,
            "scaled edge lock threshold should keep a 20-unit X move locked");

    stylus.runtime.Active().post.finalCoor = Coor(130, 100);
    aft.Process(frame);
    Require(stylus.runtime.Active().post.finalCoor.dim1 != 100,
...
void TestTiltJitterFilterMatchesTsacore() {
    Require(Solvers::Stylus::Hpp3::TiltProcess::JitterFilter1Degree(5, 7) == 6,
            "jitter filter should pull larger current tilt back by one degree");
    Require(Solvers::Stylus::Hpp3::TiltProcess::JitterFilter1Degree(7, 5) == 6,
            "jitter filter should pull smaller current tilt forward by one degree");
    Require(Solvers::Stylus::Hpp3::TiltProcess::JitterFilter1Degree(7, 7) == 7,
...
void TestCoordinateSolverUsesFixedDevicePitchTables() {
    ...
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorRow = 4;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorCol = 4;
    SetProjectionPeak(stylus.runtime.hpp3.tx1Grid.feature.projection,
                      4, 4,
                      100, 300, 100,
                      100, 300, 100);
    ...
    Require(stylus.runtime.Active().tx1.coordinate.localGridCoor.dim1 == 4608 &&
            stylus.runtime.Active().tx1.coordinate.localGridCoor.dim2 == 4608,
            "triangle path should resolve the symmetric peak to the cell center before pitch mapping");
    Require(stylus.runtime.Active().tx1.coordinate.reportGlobalCoor.dim1 == 4536,
            "dim1 report coordinate should use the fixed TSA dim1 pitch table");
    Require(stylus.runtime.Active().tx1.coordinate.reportGlobalCoor.dim2 == 4608,
...
void TestCoordinateSolverClampsGlobalCoordinateAtTopLeftEdge() {
    ...
    Require(stylus.runtime.Active().tx1.coordinate.reportGlobalCoor.dim1 == 0 &&
            stylus.runtime.Active().tx1.coordinate.reportGlobalCoor.dim2 == 0,
...
void TestCoordinateSolverClampsGlobalCoordinateAtBottomRightEdge() {
    ...
    auto& stylus = frame.stylus;
    stylus.runtime.hpp3.tx1Grid.feature.peak.valid = true;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorRow = 39;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorCol = 59;

    auto& projection = stylus.runtime.hpp3.tx1Grid.feature.projection;
    projection.Clear();
    projection.peakIdxDim1 = Solvers::Stylus::Hpp3::kGridDim - 1;
    projection.peakIdxDim2 = Solvers::Stylus::Hpp3::kGridDim - 1;
    projection.dim1[Solvers::Stylus::Hpp3::kGridDim - 1] = 300;
    projection.dim1[Solvers::Stylus::Hpp3::kGridDim - 2] = 100;
    projection.dim1[Solvers::Stylus::Hpp3::kGridDim - 3] = 50;
    projection.dim2[Solvers::Stylus::Hpp3::kGridDim - 1] = 300;
    projection.dim2[Solvers::Stylus::Hpp3::kGridDim - 2] = 100;
    projection.dim2[Solvers::Stylus::Hpp3::kGridDim - 3] = 50;
    ...
    Require(stylus.runtime.Active().tx1.coordinate.reportGlobalCoor.dim1 == 60 * Asa::kCoorUnit - 1 &&
            stylus.runtime.Active().tx1.coordinate.reportGlobalCoor.dim2 == 40 * Asa::kCoorUnit - 1,
...
void TestCoordinateSolverUsesPhysicalTopLeftEdgeAtLocalCenter() {
    ...
    auto& projection = stylus.runtime.hpp3.tx1Grid.feature.projection;
    projection.Clear();
    projection.peakIdxDim1 = Solvers::Stylus::Hpp3::kGridDim / 2;
    projection.peakIdxDim2 = Solvers::Stylus::Hpp3::kGridDim / 2;
    projection.dim1[4] = 300;
    projection.dim1[5] = 100;
    projection.dim1[6] = 50;
    projection.dim2[4] = 300;
    ...
    stylus.runtime.hpp3.tx1Grid.feature.dim1SelectedPeakOnEdge = true;
    stylus.runtime.hpp3.tx1Grid.feature.dim2SelectedPeakOnEdge = true;
    stylus.runtime.hpp3.tx1Grid.feature.dim1SelectedPeakNetSignal = 300;
    stylus.runtime.hpp3.tx1Grid.feature.dim2SelectedPeakNetSignal = 300;
    ...
    Require(stylus.runtime.Active().signal.dim1EdgeActive && stylus.runtime.Active().signal.dim2EdgeActive,
...
void TestTiltProcessUsesTx2RefinedCoordinate() {
    ...
    stylus.runtime.hpp3.rawGrid.grid.tx2.valid = true;
    stylus.runtime.hpp3.tx1Grid.feature.peak.valid = true;
    stylus.runtime.hpp3.tx1Grid.feature.peakSignal = 320;
    stylus.runtime.hpp3.tx2Grid.feature.peakSignal = 777;
    SetProjectionPeak(stylus.runtime.hpp3.tx1Grid.feature.projection,
                      4, 4,
                      100, 300, 100,
                      120, 320, 120);
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorRow = 4;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorCol = 4;
    stylus.runtime.hpp3.rawGrid.grid.tx2.anchorRow = 6;
    stylus.runtime.hpp3.rawGrid.grid.tx2.anchorCol = 5;
    stylus.runtime.hpp3.tx2Grid.feature.refinedLocalCoor = {1234, 2345, true};

    Solvers::Stylus::Hpp3::CoordinateSolver solver;
    Solvers::Stylus::Hpp3::TiltProcess tilt;
    ProcessHpp3(solver, frame);
    ProcessHpp3(tilt, frame);

    Require(stylus.runtime.Active().tx2.coordinate.localGridCoor.valid,
            "tilt process should accept TX2 refined coordinate");
    Require(stylus.runtime.Active().tx2.coordinate.localGridCoor.dim1 == 1234 &&
            stylus.runtime.Active().tx2.coordinate.localGridCoor.dim2 == 2345,
            "tilt process should consume TX2 refined coordinate directly");
    Require(stylus.runtime.Active().tx2.coordinate.reportGlobalCoor.dim1 == 1234 + Asa::kCoorUnit &&
            stylus.runtime.Active().tx2.coordinate.reportGlobalCoor.dim2 == 2345 + 2 * Asa::kCoorUnit,
            "tilt process should map TX2 refined coordinate into anchored global grid space");
    Require(stylus.runtime.Active().signal.signalY == 777,
...
void TestTiltProcessClampsAnchoredCoordinatesBeforeTiltDiff() {
    ...
    stylus.runtime.hpp3.tx1Grid.feature.peak.valid = true;
    stylus.runtime.hpp3.tx1Grid.feature.peakSignal = 320;
    stylus.runtime.hpp3.tx2Grid.feature.peakSignal = 320;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorRow = 0;
    stylus.runtime.hpp3.rawGrid.grid.tx1.anchorCol = 0;
    stylus.runtime.hpp3.rawGrid.grid.tx2.anchorRow = 0;
    stylus.runtime.hpp3.rawGrid.grid.tx2.anchorCol = 0;
    ...
    projection.dim2[1] = 100;
    projection.dim2[2] = 50;
    stylus.runtime.hpp3.tx2Grid.feature.refinedLocalCoor = Coor(0, 0);
    ...
    Require(stylus.runtime.Active().tx2.coordinate.reportGlobalCoor.dim1 == 0 &&
            stylus.runtime.Active().tx2.coordinate.reportGlobalCoor.dim2 == 0,
            "tilt process should clamp TX2 global coordinate into the physical sensor space");
    Require(stylus.runtime.Active().tilt.diffDim1 == 0 &&
            stylus.runtime.Active().tilt.diffDim2 == 0,
...
void TestTiltProcessProducesTiltAndPostOutput() {
    ...
    stylus.runtime.hpp3.rawGrid.grid.tx2.anchorCol = 4;
    stylus.runtime.hpp3.tx1Grid.feature.peak.valid = true;
    stylus.runtime.hpp3.tx1Grid.feature.peakSignal = 420;
    stylus.runtime.hpp3.tx2Grid.feature.peakSignal = 280;
    SetProjectionPeak(stylus.runtime.hpp3.tx1Grid.feature.projection,
                      4, 4,
                      120, 320, 120,
                      120, 320, 120);
    ...
    stylus.runtime.hpp3.tx2Grid.feature.refinedLocalCoor =
        Coor(stylus.runtime.Active().tx1.coordinate.localGridCoor.dim1 + 320,
             stylus.runtime.Active().tx1.coordinate.localGridCoor.dim2 - 192);

    Solvers::Stylus::Hpp3::TiltProcess tilt;
    ProcessHpp3(tilt, frame);
    stylus.runtime.Active().decision.inRangeCandidate = true;
    stylus.runtime.Active().decision.tipDownCandidate = true;
    stylus.runtime.Active().pressure.outputPressure = 256;

    Require(stylus.runtime.Active().tilt.valid,
            "tilt process should mark tilt valid when TX1/TX2 coordinates exist");
    Require(stylus.runtime.Active().tilt.preTiltDim1 != 0 || stylus.runtime.Active().tilt.preTiltDim2 != 0,
            "tilt process should generate a non-zero pre-filter tilt for a shifted TX2 coordinate");
    Require(stylus.runtime.Active().post.point.tiltValid,
            "post processor should propagate tilt validity to the output point");
    Require(stylus.runtime.Active().post.point.tiltX == stylus.runtime.Active().tilt.reportTiltDim1 &&
            stylus.runtime.Active().post.point.tiltY == stylus.runtime.Active().tilt.reportTiltDim2,
            "post processor should publish tilt output from the tilt runtime block");
    Require(stylus.runtime.Active().post.point.tiltMagnitude > 0.0f,
...
void TestTiltProcessKeepsLastFrameWhenTx2Invalid() {
    ...
    firstStylus.runtime.hpp3.rawGrid.grid.tx2.valid = true;
    firstStylus.runtime.hpp3.rawGrid.grid.tx1.anchorRow = 4;
    firstStylus.runtime.hpp3.rawGrid.grid.tx1.anchorCol = 4;
    firstStylus.runtime.hpp3.rawGrid.grid.tx2.anchorRow = 4;
    firstStylus.runtime.hpp3.rawGrid.grid.tx2.anchorCol = 4;
    firstStylus.runtime.hpp3.tx1Grid.feature.peak.valid = true;
    firstStylus.runtime.hpp3.tx1Grid.feature.peakSignal = 420;
    firstStylus.runtime.hpp3.tx2Grid.feature.peakSignal = 280;
    SetProjectionPeak(firstStylus.runtime.hpp3.tx1Grid.feature.projection,
                      4, 4,
                      120, 320, 120,
                      120, 320, 120);
    ProcessHpp3(solver, first);
    firstStylus.runtime.hpp3.tx2Grid.feature.refinedLocalCoor =
        Coor(firstStylus.runtime.Active().tx1.coordinate.localGridCoor.dim1 + 320,
             firstStylus.runtime.Active().tx1.coordinate.localGridCoor.dim2 - 192);
    ProcessHpp3(tilt, first);

    const int16_t prevPreTiltX = firstStylus.runtime.Active().tilt.preTiltDim1;
    const int16_t prevPreTiltY = firstStylus.runtime.Active().tilt.preTiltDim2;
    const int16_t prevTiltX = firstStylus.runtime.Active().tilt.reportTiltDim1;
    const int16_t prevTiltY = firstStylus.runtime.Active().tilt.reportTiltDim2;

    Solvers::HeatmapFrame second{};
    auto& secondStylus = second.stylus;
    secondStylus.SnapshotBtInput(256, 2, true);
    secondStylus.runtime.hpp3.rawGrid.grid.tx2.valid = false;
    secondStylus.runtime.hpp3.rawGrid.grid.tx1.anchorRow = 4;
    secondStylus.runtime.hpp3.rawGrid.grid.tx1.anchorCol = 4;
    secondStylus.runtime.hpp3.tx1Grid.feature.peak.valid = true;
    secondStylus.runtime.hpp3.tx1Grid.feature.peakSignal = 420;
    SetProjectionPeak(secondStylus.runtime.hpp3.tx1Grid.feature.projection,
                      4, 4,
                      120, 320, 120,
                      120, 320, 120);
    ProcessHpp3(solver, second);
    ProcessHpp3(tilt, second);

    Require(secondStylus.runtime.Active().tilt.preTiltDim1 == prevPreTiltX &&
            secondStylus.runtime.Active().tilt.preTiltDim2 == prevPreTiltY &&
            secondStylus.runtime.Active().tilt.reportTiltDim1 == prevTiltX &&
            secondStylus.runtime.Active().tilt.reportTiltDim2 == prevTiltY,
            "tilt process should keep the previous frame tilt when TX2 is invalid");

    Require(secondStylus.runtime.Active().post.point.tiltValid,
            "post point tilt should stay valid when TX2 drops and tilt is held");
    Require(secondStylus.runtime.Active().post.point.tiltX == prevTiltX &&
            secondStylus.runtime.Active().post.point.tiltY == prevTiltY,
            "post point tiltX/Y should match held report tilt when TX2 is invalid");
    Require(secondStylus.runtime.Active().post.point.preTiltX == prevPreTiltX &&
            secondStylus.runtime.Active().post.point.preTiltY == prevPreTiltY,
...
void TestRawHistoryShiftAndLatestAtZero() {
    ...
    Solvers::HeatmapFrame frame{};
    auto& stylus = frame.stylus;
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(1000, 2000);
    stylus.runtime.Active().pressure.outputPressure = 300;

    filter.Process(frame);
    Require(stylus.runtime.Active().post.postCoor.dim1 == 1000 &&
            stylus.runtime.Active().post.postCoor.dim2 == 2000,
            "first frame postCoor should match raw input");
    Require(stylus.runtime.Active().post.finalCoor.dim1 == 1000 &&
            stylus.runtime.Active().post.finalCoor.dim2 == 2000,
            "first frame finalCoor should passthrough");

    // Second frame with different coor
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(1100, 2100);
    filter.Process(frame);
    Require(stylus.runtime.Active().post.postCoor.dim1 == 1100 &&
            stylus.runtime.Active().post.postCoor.dim2 == 2100,
            "second frame postCoor should match latest raw (history < 3 frames)");

    // Third frame — avg3 activates
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(1200, 2200);
    filter.Process(frame);
    const int32_t expectedDim1 = (1000 + 1100 + 1200) / 3;
    const int32_t expectedDim2 = (2000 + 2100 + 2200) / 3;
    Require(stylus.runtime.Active().post.postCoor.dim1 == expectedDim1 &&
            stylus.runtime.Active().post.postCoor.dim2 == expectedDim2,
...
void TestPenDownAnchorCapturedOnce() {
    ...
    Solvers::HeatmapFrame frame{};
    auto& stylus = frame.stylus;
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(1500, 2500);
    stylus.runtime.Active().pressure.outputPressure = 100;
    filter.Process(frame);

    // Second frame: still pressure active, coordinate changed, anchor should NOT change
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(1600, 2600);
    stylus.runtime.Active().pressure.outputPressure = 200;
    filter.Process(frame);

    // Third frame with a different coordinate
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(1700, 2700);
    stylus.runtime.Active().pressure.outputPressure = 300;
    filter.Process(frame);
    ...
    Require(stylus.runtime.Active().post.postCoor.dim1 == 1600 &&
            stylus.runtime.Active().post.postCoor.dim2 == 2600,
...
void TestAvg3WarmUpPassthrough() {
    ...
    Solvers::HeatmapFrame frame{};
    auto& stylus = frame.stylus;
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(800, 1800);
    stylus.runtime.Active().pressure.outputPressure = 300;

    // Frame 1: history count = 1, less than 3
    filter.Process(frame);
    Require(stylus.runtime.Active().post.postCoor.dim1 == 800 &&
            stylus.runtime.Active().post.postCoor.dim2 == 1800,
            "warm-up frame 1 postCoor should passthrough raw");

    // Frame 2: history count = 2, still less than 3
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(880, 1880);
    filter.Process(frame);
    Require(stylus.runtime.Active().post.postCoor.dim1 == 880 &&
            stylus.runtime.Active().post.postCoor.dim2 == 1880,
...
void TestAvg3ThreePointAverage() {
    ...
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(300, 900);
    stylus.runtime.Active().pressure.outputPressure = 300;
    filter.Process(frame);

    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(600, 1200);
    filter.Process(frame);

    // On 3rd frame, avg3 = (300+600+900)/3 = 600, (900+1200+1500)/3 = 1200
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(900, 1500);
    filter.Process(frame);
    Require(stylus.runtime.Active().post.postCoor.dim1 == 600 &&
            stylus.runtime.Active().post.postCoor.dim2 == 1200,
            "3-point average should be (x0+x1+x2)/3");

    // 4th frame: avg3 = (600+900+1000)/3, (1200+1500+1600)/3
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(1000, 1600);
    filter.Process(frame);
    const int32_t expectedAvgX = (600 + 900 + 1000) / 3;
    const int32_t expectedAvgY = (1200 + 1500 + 1600) / 3;
    Require(stylus.runtime.Active().post.postCoor.dim1 == expectedAvgX &&
            stylus.runtime.Active().post.postCoor.dim2 == expectedAvgY,
...
void TestPredictedCoorQuadraticExtrapolation() {
    ...
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(100, 200);
    stylus.runtime.Active().pressure.outputPressure = 300;
    filter.Process(frame);

    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(110, 220);
    filter.Process(frame);

    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(120, 240);
    filter.Process(frame);

    // predicted = 3*x0 - 3*x1 + x2 = 3*120 - 3*110 + 100 = 360 - 330 + 100 = 130
    // predicted = 3*y0 - 3*y1 + y2 = 3*240 - 3*220 + 200 = 720 - 660 + 200 = 260
    Require(stylus.runtime.Active().post.predictedCoor.valid,
            "predicted coordinate should be valid after 3 frames");
    Require(stylus.runtime.Active().post.predictedCoor.dim1 == 130 &&
            stylus.runtime.Active().post.predictedCoor.dim2 == 260,
            "predicted = 3*x0 - 3*x1 + x2 (quadratic extrapolation)");

    // 4th frame: (135, 270)
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(135, 270);
    filter.Process(frame);
    // predicted = 3*135 - 3*120 + 110 = 405 - 360 + 110 = 155
    Require(stylus.runtime.Active().post.predictedCoor.dim1 == 155 &&
            stylus.runtime.Active().post.predictedCoor.dim2 == 310,
...
void TestLinearCorrectionUsesRawInputUntilLineConstraintApplies() {
    ...
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(1080, 2000);
    filter.Process(frame);

    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(1010, 2010);
    filter.Process(frame);

    // After 3 frames, avg3 smooths the jitter
    // avg3[0] = (1000+1080+1010)/3 = 1030
    Require(stylus.runtime.Active().post.postCoor.dim1 == 1030,
    ...
    Require(stylus.runtime.Active().post.finalValid,
            "finalCoor should be valid when processing avg3 input");
    Require(stylus.runtime.Active().post.finalCoor.dim1 == 1010 &&
            stylus.runtime.Active().post.finalCoor.dim2 == 2010,
...
void TestHoverPathKeepsAvg3OutOfFinalCoor() {
    ...
    Solvers::HeatmapFrame frame{};
    auto& stylus = frame.stylus;
    stylus.runtime.Active().pressure.outputPressure = 0;
    ...
    Require(stylus.runtime.Active().post.finalCoor.dim1 == 1200 &&
            stylus.runtime.Active().post.finalCoor.dim2 == 2200,
...
void TestPressureInactiveResetsHistoryState() {
    ...
    for (int i = 0; i < 4; ++i) {
        stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(500 + i * 10, 1500 + i * 10);
        stylus.runtime.Active().pressure.outputPressure = 300;
        filter.Process(frame);
    }
    ...
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(999, 1999);
    stylus.runtime.Active().pressure.outputPressure = 0;
    filter.Process(frame);

    Require(stylus.runtime.Active().post.finalCoor.dim1 == 999 &&
            stylus.runtime.Active().post.finalCoor.dim2 == 1999,
            "pressure inactive should passthrough raw coordinate");
    Require(stylus.runtime.Active().post.linearFilterState == 0,
    ...
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(700, 1700);
    stylus.runtime.Active().pressure.outputPressure = 300;
    filter.Process(frame);
    Require(stylus.runtime.Active().post.finalCoor.dim1 == 700 &&
            stylus.runtime.Active().post.finalCoor.dim2 == 1700,
...
void TestIirCoefSelectionUsesRawMappedHistorySpeed() {
    ...
    for (int i = 0; i < 3; ++i) {
        stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = Coor(rawX[i], 2000);
        stylus.runtime.Active().post.finalCoor = Coor(1000, 2000);
        stylus.runtime.Active().post.finalValid = true;
