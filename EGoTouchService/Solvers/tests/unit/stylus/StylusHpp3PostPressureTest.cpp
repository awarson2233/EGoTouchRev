...
using Solvers::Stylus::Hpp3::Hpp3PostPressureProcess;
using Solvers::Stylus::Hpp3::PressureSolver;
...
auto ProcessHpp3(ProcessT& process, HeatmapFrame& frame) {
...
HeatmapFrame MakeFrame(int32_t x,
    ...
    stylus.runtime.Active().tx1.coordinate.reportGlobalCoor = {x, y, true};
    stylus.runtime.Active().signal.signalX = signalX;
    stylus.runtime.Active().signal.signalY = signalY;
    stylus.runtime.Active().signal.maxRawPeak = signalX > signalY ? signalX : signalY;

    stylus.runtime.Active().pressure.rawPressure = pressure;
    stylus.runtime.Active().pressure.mappedPressure = pressure;
    stylus.runtime.Active().pressure.outputPressure = pressure;
    stylus.runtime.Active().pressure.btSample.pressure.fill(0);
    stylus.runtime.Active().pressure.btSample.pressure[3] = pressure;
    stylus.runtime.Active().pressure.btSample.hasSample = true;

    stylus.runtime.Active().decision.inRangeCandidate = true;
    stylus.runtime.Active().decision.tipDownCandidate = pressure != 0;
    stylus.runtime.Active().decision.authoritativeDown = pressure != 0;
    return frame;
}

void SetBtFreq(HeatmapFrame& frame, uint8_t freq1, uint8_t freq2) {
    frame.stylus.runtime.Active().pressure.btSample.freq1 = freq1;
    frame.stylus.runtime.Active().pressure.btSample.freq2 = freq2;
    frame.stylus.runtime.Active().pressure.btSample.hasFreq = true;
}
...
void SetEdge(HeatmapFrame& frame,
             ...
             uint16_t dim1Signal,
             uint16_t dim2Signal) {
    auto& signal = frame.stylus.runtime.Active().signal;
...
void TestDisabledModulePreservesRuntime() {
    ...
    frame.stylus.runtime.Active().decision.tipDownCandidate = false;
    ProcessHpp3(process, frame);

    Require(frame.stylus.runtime.Active().pressure.outputPressure == 123,
            "disabled post-pressure should preserve pressure");
    Require(!frame.stylus.runtime.Active().decision.tipDownCandidate,
...
void TestNoPressDisabledDoesNotInjectPressure() {
    ...
    Require(frame.stylus.runtime.Active().pressure.outputPressure == 0,
...
void TestNoPressCandidateRetainsPreviousPressureOrFallback() {
    ...
    ProcessHpp3(process, first);

    auto retained = MakeFrame(1000, 1000, 0);
    retained.stylus.runtime.Active().decision.tipDownCandidate = true;
    retained.stylus.runtime.Active().decision.authoritativeDown = true;
    ProcessHpp3(process, retained);

    Require(retained.stylus.runtime.Active().pressure.outputPressure == 321,
    ...
    fallback.stylus.runtime.Active().decision.tipDownCandidate = true;
    fallback.stylus.runtime.Active().decision.authoritativeDown = true;
    ProcessHpp3(fresh, fallback);

    Require(fallback.stylus.runtime.Active().pressure.outputPressure == 10,
...
void TestPostPressurePublishesFinalPressureForCommit() {
    ...
    frame.stylus.runtime.Active().post.finalValid = true;
    ProcessHpp3(process, frame);
    commit.Commit(frame);

    Require(frame.stylus.runtime.Active().post.finalPressure == 321,
            "post-pressure should publish output pressure as final pressure");
    Require(frame.stylus.runtime.Active().post.point.rawPressure == 321,
            "post-pressure should publish raw pressure into the solve point");
    Require(frame.stylus.runtime.Active().post.point.mappedPressure == 321,
            "post-pressure should publish mapped pressure into the solve point");
    Require(frame.stylus.runtime.Active().post.point.pressure == 321,
...
void TestPressureSolverUsesTsacoreOnCellOrderAndResetsOnNewPacket() {
    ...
    Require(frame.stylus.runtime.Active().pressure.rawPressure == 100,
            "OnCell slot 0 should use pressure[0]");
    ProcessHpp3(solver, frame);
    Require(frame.stylus.runtime.Active().pressure.rawPressure == 200,
            "OnCell slot 1 should use pressure[1]");
    ProcessHpp3(solver, frame);
    Require(frame.stylus.runtime.Active().pressure.rawPressure == 200,
            "OnCell slot 2 should repeat pressure[1]");
    ProcessHpp3(solver, frame);
    Require(frame.stylus.runtime.Active().pressure.rawPressure == 300,
            "OnCell slot 3 should use pressure[2]");
    ProcessHpp3(solver, frame);
    Require(frame.stylus.runtime.Active().pressure.rawPressure == 400,
    ...
    ProcessHpp3(solver, frame);
    Require(frame.stylus.runtime.Active().pressure.rawPressure == 500,
...
void TestLookaheadHoverGateClearsPressureAndDecision() {
    ...
    ProcessHpp3(solver, down);
    Require(down.stylus.runtime.Active().pressure.outputPressure != 0,
            "nonzero packet should produce pressure before look-ahead hover");
    Require(down.stylus.runtime.Active().decision.tipDownCandidate,
    ...
    ProcessHpp3(solver, hover);
    Require(hover.stylus.runtime.Active().pressure.lookaheadHoverGate,
            "last pressure slot zero should set lookahead hover gate");
    Require(hover.stylus.runtime.Active().pressure.outputPressure == 0,
            "last pressure slot zero should clear output pressure immediately");
    Require(!hover.stylus.runtime.Active().decision.tipDownCandidate,
...
void TestLookaheadHoverGateBlocksPostPressureReinjection() {
    ...
    hover.stylus.runtime.Active().pressure.lookaheadHoverGate = true;
    hover.stylus.runtime.Active().decision.tipDownCandidate = true;
    hover.stylus.runtime.Active().decision.authoritativeDown = true;
    ProcessHpp3(process, hover);
...
void TestBtPressSignalSuppressSkipsEdgeFrames() {
    ...
    Require(frame.stylus.runtime.Active().pressure.outputPressure == 200,
...
void TestBtPressSignalSuppressLatchExitsOnlyAboveThreshold() {
    ...
    ProcessHpp3(solver, enter);
    Require(enter.stylus.runtime.Active().pressure.outputPressure == 0,
    ...
    ProcessHpp3(solver, equalExit);
    Require(equalExit.stylus.runtime.Active().pressure.outputPressure == 0,
    ...
    ProcessHpp3(solver, aboveExit);
    Require(aboveExit.stylus.runtime.Active().pressure.outputPressure == 200,
            "BT pressure suppression should clear only after the exit threshold");
    Require(aboveExit.stylus.runtime.Active().decision.tipDownCandidate,
...
void TestFakePressureDecreaseBuckets() {
    {
        ...
        ProcessHpp3(process, second);
        Require(second.stylus.runtime.Active().pressure.outputPressure == 0,
    ...
    }
    {
        ...
        ProcessHpp3(process, second);
        Require(second.stylus.runtime.Active().pressure.outputPressure == 400,
    ...
    }
    {
        ...
        ProcessHpp3(process, second);
        Require(second.stylus.runtime.Active().pressure.outputPressure == 533,
                "movement 301..500 should synthesize first two-frame fake pressure value");
        auto third = MakeFrame(800, 0, 0);
        ProcessHpp3(process, third);
        Require(third.stylus.runtime.Active().pressure.outputPressure == 266,
    ...
    }
    {
        ...
        ProcessHpp3(process, second);
        Require(second.stylus.runtime.Active().pressure.outputPressure == 600,
                "movement >500 should synthesize first three-frame fake pressure value");
        auto third = MakeFrame(1200, 0, 0);
        ProcessHpp3(process, third);
        Require(third.stylus.runtime.Active().pressure.outputPressure == 400,
                "three-frame fake pressure should decay using previous processed output");
        auto fourth = MakeFrame(1800, 0, 0);
        ProcessHpp3(process, fourth);
        Require(fourth.stylus.runtime.Active().pressure.outputPressure == 0,
...
void TestBtFreqShiftDebounceSuppressesFakePressure() {
    ...
    ProcessHpp3(process, shifted);

    Require(shifted.stylus.runtime.Active().pressure.outputPressure == 0,
...
void TestBothEdgeEnterUsesTwoThirdsThreshold() {
    ...
    ProcessHpp3(process, above);
    Require(above.stylus.runtime.Active().pressure.outputPressure == 200,
    ...
    ProcessHpp3(process, below);
    Require(below.stylus.runtime.Active().pressure.outputPressure == 0,
