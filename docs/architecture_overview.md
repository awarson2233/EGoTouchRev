...
- **ServiceHost**：模块生命周期编排，IPC 命令分发，配置权威管理
...
| **Config Framework** | YAML 配置管理框架<br/>编译时绑定 + 运行时零开销 | `Common/include/config/` |
| **DVRCore** | DVR2 二进制录制/回放、CSV 导出 | `Common/DVRCore/` |
| **IPCCore** | IPC 协议 ABI、Named Pipe、Shared Memory | `Common/IPCCore/` |

**配置框架核心流程**：
```
YAML 文件 → ConfigStore → ConfigBinder 绑定 → SchemaValidator 校验 → apply() 写入成员
```
...
| **配置管理** | `GetConfigSnapshot`, `ApplyConfigPatch`, `PersistConfig` | Service-owned 配置 |
